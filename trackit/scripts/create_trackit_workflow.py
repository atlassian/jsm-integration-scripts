        ...
        if response.status_code < 299:
            ...
            try:
                ...
                if response_map:
                    ...
                    if flow_id:
                        alert_api_url = args['jsmUrl'] + "/jsm/ops/integration/v2/alerts/" + alert_id + "/details"
                        content = {
                            "details":
                                {
                                    "workflow_id": flow_id
                        ...
                        if alert_response.status_code < 299:
                            logging.info(LOG_PREFIX + " Successfully sent to Jira Service Management")
                            logging.debug(
                                LOG_PREFIX + " TrackIt response: " + str(alert_response.content) + " " + str(alert_response.status_code))
                        else:
                            logging.warning(
                                LOG_PREFIX + " Could not execute at Jira Service Management; response: " + str(alert_response.content) + " status code: " + str(alert_response.status_code))
                    else:
                        logging.warning(
