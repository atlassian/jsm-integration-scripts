...
func configureLogger() log.Logger {
	...
	if err != nil {
		fmt.Println("Could not create log file \""+logFilePath+"\", will log to \"/tmp/nagios2jsm.log\" file. Error: ", err)

		fileTmp, errTmp := os.OpenFile("/tmp/nagios2jsm.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
...
func http_post() {
	apiUrl := configParameters["jsmApiUrl"] + "/jsm/ops/integration/v1/json/opsview"
	target := "Jsm"
...
func parseFlags() map[string]string {
	...
	jsmApiUrl := flag.String("jsmApiUrl", "", "jsmApiUrl")
	...
	if *jsmApiUrl != "" {
		if strings.Contains(*jsmApiUrl, "https://") {
			configParameters["jsmApiUrl"] = *jsmApiUrl
		} else {
			configParameters["jsmApiUrl"] = "https://" + (*jsmApiUrl)
		}
	}
