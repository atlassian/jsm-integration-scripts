...
def add_details(issue_number, alert_id):
    endpoint = args['jsmUrl'] + '/jsm/ops/integration/v2/alerts/' + alert_id + '/details'
    headers = {
            ...
            <namesp1:MRWebServices__createIssue
                ...
                <args xsi:type='namesp2:SOAPStruct'>
                    <projfields xsi:type='namesp2:SOAPStruct'>
                        <JSM__bAlert__bAlias>{2}</JSM__bAlert__bAlias>
                    </projfields>
...
def main():
    ...
    if not problem_workspace_id and not incident_workspace_id:
        logging.error(LOG_PREFIX + 'Cannot find both of the incidentWorkspaceId and problemWorkspaceId either in the '
                                   'configuration file or from the Jira Service Management payload. Please fill one of the incident '
    ...
    if mapped_action == 'createIncident':
        ...
        if not issue_type:
            logging.error(LOG_PREFIX + 'Cannot obtain issueType from the Jira Service Management payload. '
                                       'Please make sure your integrations settings are correct in Jira Service Management.')
            return
