public class JsmAlertTrigger : ActiveTrigger<Force__Case>
{
	public override void Execute(TransactionContext<Force__Case> trigger)
	{
	    const string JSM_API_KEY = "<YOUR JSM MAGENTRIX INTEGRATION API KEY>";
        const string JSM_URL = "https://api.atlassian.com/jsm/ops/integration/v1/json/magentrix?apiKey=" + JSM_API_KEY;
	    ...
	    if (trigger.IsAfter)
        {
            foreach (var rec in trigger.Records)
            {
                ...
                client.UploadString(JSM_URL, postData);
            }
        }
	}	
}
