...
def send_acknowledge_request(content_map):
    ...
    if source and source["name"].lower().startswith("icinga"):
        logging.warning("Jira Service Management alert is already acknowledged by icinga. Discarding!!!")
    else:
        url_path = "/v1/actions/acknowledge-problem"
        content_map["comment"] = "Acknowledged by " + alert["username"] + " via Jira Service Management"
...
def send_unacknowledge_request(content_map):
    ...
    content_map["comment"] = "UnAcknowledged by " + alert["username"] + " via Jira Service Management"
    content_map["author"] = alert["username"]
    content_map["notify"] = True
    post_to_icingaApi(url_path, content_map)
...
def send_take_ownership_request(content_map):
    ...
    content_map["author"] = "Jira Service Management"
    post_to_icingaApi(url_path, content_map)
...
def attach(is_service_alert):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        attach_alert_url = args['jsmUrl'] + "/jsm/ops/integration/v2/alerts/" + alert_from_jsm[
...
def parse_from_details(key):
    if key in alert_from_jsm["details"].keys():
        return alert_from_jsm["details"][key]
    return ""
...
def main():
    ...
    get_alert_url = args['jsmUrl'] + "/jsm/ops/integration/v2/alerts/" + alert["alertId"] + "?alertIdentifierType=id"
    ...
    if "data" in content.keys():
        ...
        logging.warning(
            LOG_PREFIX + " Alert with id " + alert["alertId"] + " does not exist in Jira Service Management. It is probably deleted.")
