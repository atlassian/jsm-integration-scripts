...
def main():
    ...
    if alert_id:
        jsm_alert_api_url = args['jsmUrl'] + "/jsm/ops/integration/v2/alerts/" + alert_id
        jsm_alert_api_headers = {
            "Content-Type": "application/json",
            "Accept-Language": "application/json",
            "Authorization": "GenieKey " + args['apiKey']
        }
        alert_response = requests.get(jsm_alert_api_url, headers=jsm_alert_api_headers, timeout=timeout)
        if alert_response.status_code < 299 and alert_response.json()['data']:
            ...
            if action == "Acknowledge":
                if source and str(source['name']).lower() == "zenoss":
                    logging.warning("JSM alert is already acknowledged by zenoss. Discarding!!!")
                    discard_action = True
                else:
                    post_params.update({"method": "acknowledge"})
            elif action == "Close":
                if source and str(source['name']).lower() == "zenoss":
                    logging.warning("JSM alert is already closed by zenoss. Discarding!!!")
                    discard_action = True
                else:
