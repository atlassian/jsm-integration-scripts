...
var configParameters = map[string]string{"apiKey": API_KEY,
	...
	"vcsa2jsm.logger":"warning",
	"vcsa2jsm.http.proxy.enabled" : "false",
	"vcsa2jsm.http.proxy.port" : "1111",
	"vcsa2jsm.http.proxy.host": "localhost",
	"vcsa2jsm.http.proxy.protocol":"http",
	"vcsa2jsm.http.proxy.username": "",
	"vcsa2jsm.http.proxy.password": ""}
...
func configureLogger ()log.Logger{
	level := configParameters["vcsa2jsm.logger"]
	var logFilePath = parameters["logPath"]

	if len(logFilePath) == 0 {
		logFilePath = "/var/log/jsm/vcsa2jsm.log"
	}
	...
	if err != nil {
		...
		fileTmp, errTmp := os.OpenFile("/tmp/vcsa2jsm.log", os.O_CREATE | os.O_WRONLY | os.O_APPEND, 0666)
...
func getHttpClient (timeout int) *http.Client {
	...
	var proxyEnabled = configParameters["vcsa2jsm.http.proxy.enabled"]
	var proxyHost = configParameters["vcsa2jsm.http.proxy.host"]
	var proxyPort = configParameters["vcsa2jsm.http.proxy.port"]
	var scheme = configParameters["vcsa2jsm.http.proxy.protocol"]
	var proxyUsername = configParameters["vcsa2jsm.http.proxy.username"]
	var proxyPassword = configParameters["vcsa2jsm.http.proxy.password"]
...
func http_post()  {
	...
	var logPrefix = "[VCSA2JiraServiceManagement] "

	apiUrl := configParameters["jsm.api.url"] + "/jsm/ops/integration/v1/json/vcsa"
