...
def attach(entity):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        attach_alert_url = parse_field('jsmUrl', True) + "/jsm/ops/integration/v2/alerts/" + alert_from_jsm[
...
def main():
    ...
    get_alert_url = parse_field('jsmUrl', True) + "/jsm/ops/integration/v2/alerts/" + alert["alertId"]
    ...
    if "data" in content.keys():
        ...
        if action == "Create":
            ...
            if source and source["name"].lower().startswith("nagios"):
                logging.warning("Jira Service Management alert is already acknowledged by nagios. Discarding!!!")
                discard_action = True
            else:
                post_params['com_data'] = "Acknowledged by " + alert["username"] + " via Jira Service Management"
            ...
            if source and source["name"].lower().startswith("nagios"):
                logging.warning("Jira Service Management alert is already unacknowledged by nagios. Discarding!!!")
                discard_action = True
            else:
                if service:
