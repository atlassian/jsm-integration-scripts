...
def add_details(bus_ob_public_id):
    endpoint = args.get('jsmUrl') + '/jsm/ops/integration/v2/alerts/' + alert_id + '/details'
    headers = {
        'Content-Type': 'application/json',
...
def create_incident():
    ...
    fields.append(BusinessObjectField(dirty=True, displayName='Customer ID', fieldId=customer_id_field_id, name='CustomerRecID', value=get_jsm_customer_id()).__dict__)
    ...
    fields.append(BusinessObjectField(dirty=True, displayName='Owned By', fieldId=owned_by_description_field_id, name='OwnedBy', value='Jsm').__dict__)
...
def set_incident_status(status):
    ...
    if not incident_has_owner(queue_message.get('incidentPublicId')):
        owned_by_description_field_id = get_field_id(incident_bus_ob_id, INCIDENT_TEMPLATE, 'Owned By')
        fields.append(BusinessObjectField(dirty=True, displayName='Owned By', fieldId=owned_by_description_field_id, name='Owned By', value='Jsm').__dict__)
...
def get_jsm_customer_id():
    jsm_customer_id = MemoryStore.lookup(CUSTOMER_JSM_ID)
    if not jsm_customer_id:
        jsm_customer_id = retrieve_jsm_customer_id()
    return jsm_customer_id


def retrieve_jsm_customer_id():
    ...
    if response.json():
        ...
        if response.status_code < 300 and response_body.get('fieldDefinitions'):
            ...
            search_payload['filters'] = [Condition(fieldId=full_name_field_id, operator='eq', value='Jsm').__dict__]
            ...
            if search_response.json():
                ...
                if search_response_business_objects and search_response_business_objects[0]:
                    jsm_cust_bus_ob_rec_id = search_response_business_objects[0].get('busObRecId')
                    if search_response.status_code < 300 and jsm_cust_bus_ob_rec_id:
                        MemoryStore.store(CUSTOMER_JSM_ID, jsm_cust_bus_ob_rec_id)
                        logging.debug("Found internal customer named 'Jira Service Management' busObRecId: " + jsm_cust_bus_ob_rec_id)
                        return jsm_cust_bus_ob_rec_id
            err_message = LOG_PREFIX + " Could not find internal customer named 'Jira Service Management'"
            logging.error(err_message)
            raise Exception(err_message)
