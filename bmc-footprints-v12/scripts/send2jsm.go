...
var configParameters = map[string]string{"apiKey": "",
	...
	"bmcFootPrints2jsm.http.proxy.port":     "1111",
	"bmcFootPrints2jsm.http.proxy.host":     "localhost",
	"bmcFootPrints2jsm.http.proxy.protocol": "http",
	"bmcFootPrints2jsm.http.proxy.username": "",
	"bmcFootPrints2jsm.http.proxy.password": ""}
var parameters = make(map[string]string)
var logPrefix = "[bmcFootPrints2jsm] "
...
var ogPrefix string = "[Jira Service Management] "
...
func main() {
	if runtime.GOOS == "windows" {
		configPath = "C:\\jsm-integration\\conf\\integration.conf"
		configPath2 = "C:\\jsm-integration\\conf\\jec-config.json"
	} else {
		configPath = "/home/jsm/jec/conf/integration.conf"
		configPath2 = "/home/jsm/jec/conf/jec-config.json"
	}
	...
	if *version != "" {
		fmt.Println("Jira Service Management - BMC FootPrints v12 Integration Package 1.0")
		return
	}
	...
	parameters["jsmAlertAlias"] = getCustomField(itemDetails.CustomFields, "Jira Service Management Alert Alias")
	...
	if len(resolution) > 0 {
		...
		if strings.HasPrefix(itemDetails.Description, ogPrefix) {
			logger.Debug("Skipping, Incident or Problem was created from Jira Service Management.")
			return
		}
...
func configureLogger() log.Logger {
	level := configParameters["bmcFootPrints2jsm.logger"]
	var logFilePath = parameters["logPath"]

	if len(logFilePath) == 0 {
		if runtime.GOOS == "windows" {
			logFilePath = "C:\\jsm\\jec\\bmcfootprintsv12\\send2jsm.log"
		} else {
	...
	if err != nil {
		...
		if runtime.GOOS == "windows" {
			tmpLogFilePath = "C:\\Windows\\Temp\\send2jsm.log"
		} else {
			tmpLogFilePath = "/tmp/send2jsm.log"
		}
...
func getHttpClient(timeout int) *http.Client {
	...
	var proxyEnabled = configParameters["bmcFootPrints2jsm.http.proxy.enabled"]
	var proxyHost = configParameters["bmcFootPrints2jsm.http.proxy.host"]
	var proxyPort = configParameters["bmcFootPrints2jsm.http.proxy.port"]
	var scheme = configParameters["bmcFootPrints2jsm.http.proxy.protocol"]
	var proxyUsername = configParameters["bmcFootPrints2jsm.http.proxy.username"]
	var proxyPassword = configParameters["bmcFootPrints2jsm.http.proxy.password"]
...
func postToJSM() {
	apiUrl := configParameters["jsm.api.url"] + "/jsm/ops/integration/v1/json/bmcfootprints"
