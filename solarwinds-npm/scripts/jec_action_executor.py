...
def main():
    ...
    get_alert_url = args['jsmUrl'] + "/jsm/ops/integration/v2/alerts/" + alert["alertId"]
    ...
    if "data" in content.keys():
        ...
        if source["type"].lower() != "solarwinds":
            definition_id = alert_from_jsm["details"]["AlertDefinitionID"]
            logging.debug("alertDefinitionID: " + str(definition_id))
            object_type = alert_from_jsm["details"]["ObjectType"]
            logging.debug("objectType: " + str(object_type))
            object_id = alert_from_jsm["details"]["ObjectID"]
            ...
            if action == "Acknowledge":
                message = alert_username + " acknowledged alert: \"" + alert_note + "\" on alert: \"" + \
                          alert_message + "\""
                comment = str_updated + " Acknowledged in Jira Service Management by " + alert_username
                ...
                message = alert_username + " added note to alert: \"" + alert_note + "\" on alert: \"" + \
                          alert[
                              "message"] + "\""
                comment = str_updated + " Updated by " + alert_username + " from Jira Service Management: " + alert_note
                add_note_solarwinds_alert(url, auth_token, object_id, comment)
