Param (
    [parameter(Mandatory=$true, HelpMessage="You must provide the Url contained in your SCOM Integration's settings in JSM UI.")][String]$Url,
    [String]$AlertID,
...
$logfile = "C:\scripts\jsm\log.csv"
$timeStampFormat = "MM-dd-yyyy HH:mm:ss"
$IntegrationLogEntry = ""


function Add-JsmIntegrationLogEntry {    
    Param (
...
if ($enableLogging){if (-not (Test-Path -LiteralPath  $logfile -PathType Leaf)){Write-Output "`"TimeStamp`",`"LogLevel`",`"SCOMAlertId`",`"LogMessage`"" | Out-File $logfile -Append};Add-JsmIntegrationLogEntry  -logLevel "INFO" -logMessage "JSM intergration script has started  for alertId $($alertID)"}
...
function Install-jsmIntegration{
    $PsCommandArgs = "-executionpolicy bypass -File `"C:\scripts\jsm\jsm.ps1`" -Url `""+$Url+"`" -AlertID `"`$Data[Default='NotPresent']/Context/DataItem/AlertId$`" -CreatedByMonitor `"`$Data[Default='NotPresent']/Context/DataItem/CreatedByMonitor$`" -ManagedEntitySource `"`$Data[Default='NotPresent']/Context/DataItem/ManagedEntityDisplayName$`" -WorkflowId `"`$Data[Default='NotPresent']/Context/DataItem/WorkflowId$`" -DataItemCreateTimeLocal `"`$Data[Default='NotPresent']/Context/DataItem/DataItemCreateTimeLocal$`" -ManagedEntityPath `"`$Data[Default='NotPresent']/Context/DataItem/ManagedEntityPath$`" -ManagedEntity `"`$Data[Default='NotPresent']/Context/DataItem/ManagedEntity$`" -MPElement `"`$MPElement$`""
    Write-host -ForegroundColor Cyan "Creating JSM Notification Channel."
    Add-SCOMNotificationChannel -ApplicationPath ($PSHOME+"\Powershell.exe") -Name "JSM" -Argument $PsCommandArgs -WorkingDirectory $PSHOME
    if ($?){write-host -ForegroundColor Green "JSM Notification Channel has been successfully created.";echo ""; echo ""} else {echo ""; write-host -ForegroundColor Yellow "Failed to create JSM Notification Channel!!!";exit}

    Write-host -ForegroundColor Cyan "Creating JSM Notification Subscriber.";echo ""
    Add-SCOMNotificationSubscriber -name "JSM" -DeviceList "JSM"
    if ($?){write-host -ForegroundColor Green "JSM Notification Subscriber has been successfully created.";echo ""; echo ""} else {echo ""; write-host -ForegroundColor Yellow "Failed to create JSM Notification Subscriber!!!";exit}

    Write-host -ForegroundColor Cyan "Creating JSM Notification Subscription.";echo ""
    $subscriber = Get-SCOMNotificationSubscriber "JSM"
    $channel = Get-SCOMNotificationChannel "JSM"
    Add-SCOMNotificationSubscription -Name ("JSM"+"_Subscription") -Subscriber $subscriber -Channel $channel
    if ($?){write-host -ForegroundColor Green "JSM Notification Subscription has been successfully created.";echo ""} else {echo ""; write-host -ForegroundColor Yellow "Failed to create JSM Notification Subscription!!!";exit}

    Write-Host -ForegroundColor Green "JSM Integration setup has completed successfully"
    exit
}


if ($install){Install-jsmIntegration}
...
if ($params.resolutionstate -match "Not Present"){
    if ($enableLogging){Add-JsmIntegrationLogEntry -logLevel "WARNING" -logMessage "The following SCOM alert $($alert.id) will not be sent to JSM as it does not contain resolutionState `"New`" or `"Closed`"."}
}
else {
    ...
    try {
        $jsm_API_Request = Invoke-RestMethod -Method Post -ContentType "application/json" -Body $json -Uri $Url
        if ($enableLogging){Add-JsmIntegrationLogEntry  -logLevel "INFO" -logMessage "Successfully posted SCOM alert to JSM API the requestId for this call is $($jsm_API_Request.requestId)."}
    }

    catch {
         if ($enableLogging){Add-JsmIntegrationLogEntry -logLevel "ERROR" -logMessage "JSM integration script has failed to POST SCOM alert details to the JSM API: Exception Type='$($_.Exception.GetType().FullName)',Exception Message='$($_.Exception.Message)'"}
    }
}
write-output $IntegrationLogEntry | Out-File $logfile -Append