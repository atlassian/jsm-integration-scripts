...
var jsmHost = 'api.atlassian.com/jsm/ops/integration';

var jsmProtocol = 'https';
var jsmPort = 443;
...
var jsmReqOpts = {
    host: jsmHost,
    port: jsmPort,
    path: '/jsm/ops/integration/v2/alerts',
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
        'Authorization': 'GenieKey ' + jsmApiKey
    },
    agent: false
};

var jsmHttp = jsmProtocol === 'https' ? https : http;
...
function createJiraIssue(event, context) {
    ...
    var jiraReqBody = {
        'fields': {
            ...
            'description': 'Issue created for Jira Service Management Alert ' + event.alert.alertId + ' from Integration ' + event.integrationId,
            'issuetype': {
                'name': 'Bug' // Make sure your JIRA project configuration(s) supports this Issue Type.
            }
        }
    };
    doApiCall(event, context, jiraHttp, jiraReqOpts, jiraReqBody, 'JIRA', 'creating issue', 201, addTagToJSMAlert);
}

function addTagToJSMAlert(event, context, jiraResBody) {
    var jsmReqBody = {
        'tags': [JIRA_ISSUE_KEY_PREFIX + jiraResBody.key]
    };
    jsmReqOpts.path = '/jsm/ops/integration/v2/alerts/' + event.alert.alertId + "/tags"
    doApiCall(event, context, jsmHttp, jsmReqOpts, jsmReqBody, 'Jira Service Management', 'adding issue key as tag to alert', 202, genericSuccessFunc);
}
