...
var configParameters = map[string]string{"apiKey": API_KEY,
	...
	"icinga2jsm.http.proxy.port" : "1111",
	"icinga2jsm.http.proxy.host": "localhost",
	"icinga2jsm.http.proxy.protocol":"http",
...
func main() {
	...
	if parameters["notification_type"] == "" {
		if logger != nil {
			logger.Error("Stopping, Icinga NOTIFICATIONTYPE param has no value, please make sure your Icinga and JSM files pass necessary parameters")
		}
...
func configureLogger ()log.Logger{
	level := configParameters["icinga2jsm.logger"]
...
func getHttpClient (timeout int) *http.Client {
	...
	var proxyEnabled = configParameters["icinga2jsm.http.proxy.enabled"]
	proxy := http.ProxyFromEnvironment

	if proxyEnabled == "true" {
		var proxyHost = configParameters["icinga2jsm.http.proxy.host"]
		var proxyPort = configParameters["icinga2jsm.http.proxy.port"]
		var scheme = configParameters["icinga2jsm.http.proxy.protocol"]
		var proxyUsername = configParameters["icinga2jsm.http.proxy.username"]
		var proxyPassword = configParameters["icinga2jsm.http.proxy.password"]
...
func http_post()  {
	...
	apiUrl := configParameters["jsm.api.url"] + "/jsm/ops/integration/v1/json/icinga"
	target := "JSM"
